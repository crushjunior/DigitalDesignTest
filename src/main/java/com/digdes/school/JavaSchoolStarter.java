...
public class JavaSchoolStarter {
    ...
    private List<Map<String, Object>> executeInsert(String request) throws Exception {
        Map<String, Object> newLine = parseRow(request); // преобразование запроса в мапу
        table.add(newLine);
    ...
    private List<Map<String, Object>> executeUpdate(String request) throws Exception {
        ...
        } else { // иначе веа нет, выбираем где обновить согласно условиям запроса
            String whereClause = requestParts[1];
            for (Map<String, Object> row : table) {
                if (checkRequest(row, whereClause)) { // если условия подходят, то обновляем данные
                    row.putAll(newLine);
    ...
    private Map<String, Object> parseRow(String request) {
        ...
        for (String part : parts) { // проходимся по массиву в котором образовались пары ключ-значение в виде строк между которыми знак равно
            ...
            String key = keyValue[0].replaceAll("'", "").trim(); // создаем строку которая будет ключом, берем первый элемент пред массива, убираем одинарные ковычки
            ...
            if (countMatchKeys == 0) {
                throw new RuntimeException("Invalid column name"); // проверили что в исходной таблице есть ключ с названием из запроса
            }
    ...
    private boolean checkRequest(Map<String, Object> existingRow, String whereClause) {
        ...
        if (!matcher.matches()) { // проверям запрос после веа, чтоб ссоотвтетсовал типу: id=2
        ...
        if (countMatchKey == 0) { // проверяем что в таблице есть такой ключ
            throw new IllegalArgumentException("This column doesn't exist: " + columnName);
        }

        return toCompare(comparableValue, existingRow.get(columnName), operator); // проверям подходят ли значения под условия
    }
    ...
    private void checkConditionsAndFillingReturnTable(List<Map<String, Object>> table, List<Map<String, Object>> deleteTable, Map<String, List<String>> whereConditions, boolean isContainsAnd) {
